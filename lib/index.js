...
class SeleniumUtils {
  ...
  buildChrome(config) {
    ...
    if (config.proxyConfig) {
      debug('buildChrome(): proxyConfig is set, need to build plugin')
      debug(config)
      return new ProxyPlugin({ proxyConfig: config.proxyConfig, chromeOptions: chromeOptions })
        .then((plugin) => this.proxyPlugin = plugin)
        .then((plugin) => debug('buildChrome(): chromeOptions are ready'))
        .then(() => new webdriver.Builder()
          .forBrowser('chrome')
          .setChromeOptions(this.proxyPlugin.chromeOptions)
          .build())
        .then((driver) => this.driver = driver)
        .then(() => this.proxyPlugin.cleanup())
        .then(() => debug('buildChrome(): driver has been built WITH proxyPlugin'))
        .then(() => this._setTimeouts())
        .then(() => this.driver)
        ;
    } else {
      debug('buildChrome(): no proxyConfig provided')
      return new webdriver.Builder()
        .forBrowser('chrome')
        .setChromeOptions(chromeOptions)
        .build()
        .then((driver) => this.driver = driver)
        .then(() => debug('buildChrome(): driver has been built WITHOUT proxyPlugin'))
        .then(() => this._setTimeouts())
        .then(() => this.driver)
        ;
    }
  ...
  buildFirefox(config) {
    ...
    if (config.profilePath) {
      debug(`buildFirefox(): profilePath is set ${config.profilePath}`);
    ...
    this.driver = new webdriver.Builder()
      ...
      .build();
    debug('buildFirefox(): driver has been built WITH existing profile');
    return this.driver;
  }
...
//
  ...
  sleep(delay) {
    return new Promise((resolve, reject) => {
      debug(`sleep(${delay}) - started`);
      setTimeout(() => {
        debug(`sleep(${delay}) - done`);
        resolve(this);
      }, delay);
    });
  }
      ...
      () => {
        let finders      = actions.map((action, index, array) => {
          ...
          if (action.wait) {
            ...
            return Promise.resolve()
              .then(() => this.driver.executeScript(action.waitJs))
              .then((scriptResult) => {
                debug('action.waitJs: scriptResult:', scriptResult);
          ...
          } else if (action.waitJsAsync) {
            //const searchTextScript = `return { test: 'test-value' };`;
            return Promise.resolve()
              .then(() => this.driver.executeAsyncScript(action.waitJsAsync))
              .then((scriptResult) => {
                debug('action.waitJsAsync: scriptResult:', scriptResult);
                return (scriptResult) ? /*Promise.resolve(*/{
                  action,
                  //element: null,
                  element: action.waitJs,
...
//passes parameters to wait element returned from waitNew
  ...
  scrollOne(bySelectors) {
    ...
    return this.waitForOneOfElements(bySelectors, this.options.timeout)//()
      .then(function ({ element, index, def }) {
        if (element) {                 // if element was found
